...
internal static partial class Program
{
    ...
    )
    {
        ...
        catch (SocketException ex)
        {
            await Console.Error.WriteLineAsync(
                $"Unable to listen on http://{hostDisplay}:{options.LiveServerPort}/: {ex.Message}"
            );
            return 1;
        }
        await Console.Error.WriteLineAsync(
            $"Live terminal: http://{hostDisplay}:{options.LiveServerPort}/"
        );
        ...
        try
        {
            ...
            var readTask = ReadLiveOutputAsync(
            ...
            while (!readTask.IsCompleted && !cancellationToken.IsCancellationRequested)
            {
                if (!connection.WaitForExit(50))
                    continue;
                var drained = await Task.WhenAny(readTask, Task.Delay(500, cancellationToken))
                    .ConfigureAwait(false);
                if (drained != readTask)
                    connection.Dispose();
                break;
    ...
    private static NativePtyOptions BuildLivePtyOptions(
        int width,
        int height,
        string[]? command,
        bool noDeleteEnvs
    ...
    private static string RenderErrorSvg(
        int width,
        int height,
        Theme theme,
    ...
    )
    {
        ...
        foreach (var ch in message)
            errorTerminal.Process(ch.ToString());
    ...
    )
    {
        ...
        using (var reader = new StreamReader(stream, Encoding.ASCII, leaveOpen: true))
        {
            ...
            if (path == "/events")
            {
                ...
                try
                {
                    await Task.Delay(Timeout.Infinite, sseClient.ClientToken).ConfigureAwait(false);
                }
                catch (OperationCanceledException)
                {
            ...
            {
                await WriteHttpAsync(stream, "200 OK", "image/svg+xml; charset=utf-8", latest())
                    .ConfigureAwait(false);
                return;
            }
            if (path == "/health")
    ...
    private static async Task ReadLiveOutputAsync(
        Stream input,
    ...
    )
    {
        ...
        while (true)
        {
            ...
            var completedTask = await Task.WhenAny(readTask, startupTimeoutTask)
                .ConfigureAwait(false);

            if (completedTask == startupTimeoutTask && !dataReceived)
            {
                publishError(
                    $"No data received from PTY within {StartupTimeoutMs}ms.\nThe command may be hanging or not producing output."
                );
    ...
    private sealed class LiveSseClient(NetworkStream stream, Action disconnected) : IAsyncDisposable
    {
        ...
        private async Task SendAsync(string svg)
        {
            ...
            try
            {
                await WriteSseAsync(stream, svg, timeout.Token).ConfigureAwait(false);
            }
            catch
            {
                disconnected();
