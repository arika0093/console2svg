...
Static PNG output from the same capture command (generated by `scripts/image-gen.sh` as `cmd-matrix.png`):

```bash
console2svg capture -o output.png -w 100 -h 24 -c -d macos-pc --timeout 2 -- cmatrix -ab
```

![A static PNG capture of cmatrix](/assets/cmd-matrix.png)
