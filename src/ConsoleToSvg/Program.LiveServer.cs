...
internal static partial class Program
{
    private static async Task<int> RunLiveServerAsync(AppOptions options, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var renderTask = RenderLiveFramesAsync(
                ...
                (backgroundSvg, chromeSvg, frameSvg) =>
                {
                    if (backgroundSvg is not null && chromeSvg is not null)
                    {
                        ...
                        BroadcastInitialSvg(clients, backgroundSvg, chromeSvg, frameSvg, options.Opacity);
                        return;
                    }
                    latestFrameSvg = frameSvg;
            ...
            if (!string.IsNullOrWhiteSpace(frameRenderOptions.BackColor)) theme = theme.WithBackground(frameRenderOptions.BackColor);
            if (!string.IsNullOrWhiteSpace(frameRenderOptions.ForeColor)) theme = theme.WithForeground(frameRenderOptions.ForeColor);
    ...
    private static async Task AcceptLiveClientsAsync(TcpListener listener, ConcurrentDictionary<int, LiveSseClient> clients, Func<(string BackgroundSvg, string ChromeSvg, string FrameSvg)> latest, string liveHtml, double opacity, CancellationToken cancellationToken)
    {
        var nextId = 0;
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            _ = ServeLiveClientAsync(client, Interlocked.Increment(ref nextId), clients, latest, liveHtml, opacity, cancellationToken);
        }
    }

    private static async Task ServeLiveClientAsync(TcpClient client, int id, ConcurrentDictionary<int, LiveSseClient> clients, Func<(string BackgroundSvg, string ChromeSvg, string FrameSvg)> latest, string liveHtml, double opacity, CancellationToken cancellationToken)
    {
        using (client)
        ...
        using (var reader = new StreamReader(stream, Encoding.ASCII, leaveOpen: true))
        {
            ...
            if (path == "/events")
            {
                ...
                await sseClient.SendInitialAsync(backgroundSvg, chromeSvg, frameSvg, opacity, cancellationToken).ConfigureAwait(false);
            ...
            if (path == "/snapshot.svg") { await WriteHttpAsync(stream, "200 OK", "image/svg+xml; charset=utf-8", latest().FrameSvg).ConfigureAwait(false); return; }
    ...
    {
        foreach (var client in clients.Values) client.TrySendInitial(backgroundSvg, chromeSvg, frameSvg, opacity);
    }

    private static void BroadcastFrameSvg(ConcurrentDictionary<int, LiveSseClient> clients, string svg)
    {
        foreach (var client in clients.Values) client.TrySendFrame(svg);
    }
    ...
    )
    {
        ...
        while (await timer.WaitForNextTickAsync(cancellationToken).ConfigureAwait(false))
        {
            ...
            var backgroundSvg = staticRendered ? null : SvgRenderer.Render(screen, backgroundRenderOptions);
    ...
    private sealed class LiveSseClient(NetworkStream stream, Action disconnected) : IAsyncDisposable
    {
        ...
        {
            await WriteSseAsync(stream, "window-opacity", opacity.ToString(CultureInfo.InvariantCulture), cancellationToken).ConfigureAwait(false);
            if (!string.IsNullOrEmpty(backgroundSvg))
            {
                await WriteSseAsync(stream, "background", backgroundSvg, cancellationToken).ConfigureAwait(false);
            }
            if (!string.IsNullOrEmpty(chromeSvg))
            {
                await WriteSseAsync(stream, "chrome", chromeSvg, cancellationToken).ConfigureAwait(false);
            }
            if (!string.IsNullOrEmpty(frameSvg))
            {
                await WriteSseAsync(stream, "frame", frameSvg, cancellationToken).ConfigureAwait(false);
            }
        }
        public void TrySendInitial(string backgroundSvg, string chromeSvg, string frameSvg, double opacity)
        {
        ...
        private async Task SendAsync(string eventName, string svg)
        {
            ...
            try { await WriteSseAsync(stream, eventName, svg, timeout.Token).ConfigureAwait(false); }
            catch { disconnected(); await stream.DisposeAsync().ConfigureAwait(false); }
            finally { Interlocked.Exchange(ref _sending, 0); }
        }
        private async Task SendInitialAndFrameAsync(string backgroundSvg, string chromeSvg, string frameSvg, double opacity)
        {
            using var timeout = new CancellationTokenSource(TimeSpan.FromSeconds(5));
            try
            {
                await WriteSseAsync(stream, "window-opacity", opacity.ToString(CultureInfo.InvariantCulture), timeout.Token).ConfigureAwait(false);
                await WriteSseAsync(stream, "background", backgroundSvg, timeout.Token).ConfigureAwait(false);
                await WriteSseAsync(stream, "chrome", chromeSvg, timeout.Token).ConfigureAwait(false);
    ...
    private static async Task WriteSseAsync(NetworkStream stream, string eventName, string svg, CancellationToken token) => await WriteBytesAsync(stream, "event: " + eventName + "\ndata: " + svg.Replace("\r", "").Replace("\n", "\ndata: ") + "\n\n", token).ConfigureAwait(false);
    ...
    private static string GetLiveHtml(double opacity) =>
        LiveHtml.Replace("{opacity}", opacity.ToString(CultureInfo.InvariantCulture), StringComparison.Ordinal);
